...
Upload.prototype._resizeOriginal = function(tmp_file, callback) {
  fs.readFile(tmp_file, function(err, buffer) {
    if (err) return callback(err);

    gm(buffer).autoOrient().colorspace('RGB').toBuffer('JPG', function(err, buffer) {
      if (err) return callback(err);

      gm(buffer).identify(function(err, exif) {
        if (err) return callback(err);

        return callback(null, buffer, exif);
      });
    });
  });
};

Upload.prototype._resizeBuffer = function(buffer, name, resize, callback) {
